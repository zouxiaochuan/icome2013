...
void SegmenterHumanFaceTemplate::segment(const cv::Mat& img, Mat_<uchar>& mask)
{
	Mat imgBGR;
	Mat imgLAB;
	Mat imgBGRo;
	...
	initial.setTo(1,prob>0.6);
	initial.setTo(0,prob<=0.6);
	...
	fgdEnergy = 1.0*fgdInit + fgdColor;
	bgdEnergy = 1.0*bgdInit + bgdColor;
...
SegmenterHumanFaceTemplate::SegmenterHumanFaceTemplate(const string& mfilename, const string& templateFilename)
{
...
SegmenterHumanFaceTemplate::~SegmenterHumanFaceTemplate()
{
}

void SegmenterHumanFaceTemplate::getPixelProbability(const cv::Mat& img, cv::Mat_<float>& prob, vector<Rect> faces)
{	
...
void SegmenterHumanFaceTemplate::_prob2energy(const cv::Mat_<float>& prob, cv::Mat_<float>& fgdEnergy, cv::Mat_<float>& bgdEnergy)
{
...
void SegmenterHumanFaceTemplate::_getColorProba(const cv::Mat& img, const cv::Mat_<int>& label, cv::Mat_<float>& colorProba)
{
